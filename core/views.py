...
def history_view(request):
    conversations = Conversation.objects.filter(Q(creator=request.user) | Q(invitee=request.user))
    # conversation_dict = list(conversations.values('id', 'creator', 'invitee', 'created_at'))
    data = []
    for i in conversations:
        ...
        if request.user == i.creator:
            tmp['name'] = i.invitee.username
            tmp['status'] = 'Incomming'
        else:
            tmp['name'] = i.creator.username
            tmp['status'] = 'Outgoing'

        if i.finished_at:
            duration = i.finished_at - i.created_at
            minutes, seconds = divmod(duration.total_seconds(), 60)
            duration_str = f"{int(minutes)} min {int(seconds)} sec"
            tmp['duration'] = duration_str
        data.append(tmp)
    context = {'conversations': data}
